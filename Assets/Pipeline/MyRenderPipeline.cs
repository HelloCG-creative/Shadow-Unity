        ...
        protected override void Render(ScriptableRenderContext context, Camera[] cameras) {
            ...
            foreach (var camera in cameras) {
                ...
                if (existValidLight) {
                    ...
                    Vector3 lightDirWS = -light.transform.forward; // SetupLightPropertiesと同じ向き
                    var lightView = CalcLightViewFromFrustum(lightDirWS, frustum8);

                    //var lightProj = CalcLightOrthoProj(lightView, frustum8);
                    var lightProj = CalcPSMFinalVP(camera, lightDirWS);
                    var lightProjGpu = GL.GetGPUProjectionMatrix(lightProj, true);

                    DrawShadowManual(context, cmd, cullingResults, lightView, lightProjGpu);*/
...
private static void GetNdcUnitCubeCorners(Vector3[] out8)
{
    ...
    for (int z = 0; z <= 1; z++)
    {
        ...
        private Matrix4x4 CalcPSM_WorldToShadowClip_Article(Camera cam, Vector3 lightDirWS, float shadowDistance, float zNearMin = 0.1f)
{
    ...
    return worldToShadowClip;
        ...
        private void DrawShadowManual(
            ...
            var sorting = new SortingSettings() { criteria = SortingCriteria.None };
            var draw = new DrawingSettings(ShadowCasterTag, sorting);
            var filter = new FilteringSettings(RenderQueueRange.opaque); // まずopaqueだけでOK
        ...
        private Matrix4x4 CalcLightViewFromFrustum(Vector3 lightDirWS, Vector3[] frustum8WS)
        {
            Vector3 center = Vector3.zero;
            for (int i = 0; i < 8; i++) center += frustum8WS[i];
            center /= 8f;

            // ライト位置は中心から逆方向に離す（距離は後でbboxから決めてもOK）
            float dist = 100f;
            Vector3 pos = center - lightDirWS.normalized * dist;

            // upが平行になるのを避ける
            Vector3 up = Mathf.Abs(Vector3.Dot(Vector3.up, lightDirWS)) > 0.95f ? Vector3.right : Vector3.up;

            return Matrix4x4.LookAt(pos, center, up);
        }

        private Matrix4x4 CalcLightOrthoProj(Matrix4x4 lightView, Vector3[] frustum8WS)
        {
            Vector3 min = new Vector3(float.PositiveInfinity, float.PositiveInfinity, float.PositiveInfinity);
            Vector3 max = new Vector3(float.NegativeInfinity, float.NegativeInfinity, float.NegativeInfinity);
            ...
            for (int i = 0; i < 8; i++)
            {
                Vector3 ls = lightView.MultiplyPoint(frustum8WS[i]);
                min = Vector3.Min(min, ls);
                max = Vector3.Max(max, ls);
            }

            // zは少し余裕を持たせる（カスケード無しの最初は大事）
            float zNear = -max.z - 50f;
            float zFar  = -min.z + 50f;

            // UnityのLookAtは右手系っぽく見えるので、ここは “符号” が環境でズレがち。
            // まずはこれで影が出るところまで合わせて、後で整理しましょう。
            return Matrix4x4.Ortho(min.x, max.x, min.y, max.y, zNear, zFar);
        }
        
        private Matrix4x4 CalcPSMFinalVP(Camera cam, Vector3 lightDirWS)
{
    // 1) Camera VP（GPU化しておく：後段のdepth/uvと合わせる）
    Matrix4x4 Vc = cam.worldToCameraMatrix;
    Matrix4x4 PcGpu = GL.GetGPUProjectionMatrix(cam.projectionMatrix, false);
    Matrix4x4 cameraVP = PcGpu * Vc;

    // 2) light dir を post-perspective へ（w=0）
    Vector4 wsLight = new Vector4(lightDirWS.x, lightDirWS.y, lightDirWS.z, 0f);
    wsLight.Normalize();
    Vector4 post = cameraVP * wsLight;
    ...
    if (Mathf.Abs(post.w) < EPS)
    {
        // directional in post-perspective space
        lightP = Matrix4x4.Ortho(-SQRT2, SQRT2, -SQRT2, SQRT2, -SQRT2, SQRT2);

        Vector3 dir = new Vector3(-post.x, -post.y, -post.z);
        Vector3 up  = Vector3.forward;
        if (Mathf.Abs(Vector3.Dot(dir.normalized, up)) > 0.99f) up = Vector3.up;

        lightV = Matrix4x4.LookAt(Vector3.zero, dir, up);
    }
    else
    {
        // point in post-perspective space
        if (post.w < 0f) post.z *= -1f;   // Pocolの簡易Z反転に寄せる

        post *= (1.0f / post.w);
        post.w = 0f;

        float radius = SQRT3;
        float dist   = new Vector3(post.x, post.y, post.z).magnitude;

        float fovRad = 2.0f * Mathf.Atan(radius / dist);
        float fovDeg = fovRad * Mathf.Rad2Deg;

        float near = Mathf.Max(dist - radius, 0.001f);
        float far  = dist + radius;

        lightP = Matrix4x4.Perspective(fovDeg, 1.0f, near, far);

        Vector3 eye = new Vector3(post.x, post.y, post.z);
        Vector3 up  = Vector3.forward;
        if (Mathf.Abs(Vector3.Dot(eye.normalized, up)) > 0.99f) up = Vector3.up;

        lightV = Matrix4x4.LookAt(eye, Vector3.zero, up);
    }

    // 3) LightVP も GPU化（Shadow map へ描く前提で true）
    Matrix4x4 PlGpu = GL.GetGPUProjectionMatrix(lightP, true);
    Matrix4x4 lightVP = PlGpu * lightV;

    // 4) Final（列ベクトルの一貫：まず cameraVP → 次に lightVP）
    Matrix4x4 final = lightVP * cameraVP;
    return final;
}
    }