...
/// シャドウによるライトの減衰度を取得する
/// </summary>
float4 GetShadowAttenuation(float4 positionWS, float dotNL)
{
    // ワールド空間からLVP空間へと変換する
    ...
    // ピクセルがある位置のLVP空間上の深度（生のNDC深度＝ハードウェア深度と同じ規約）
    float zInLVP = positionLVP.z / positionLVP.w;
    // シャドウマップ（Depthフォーマット）に書き込まれている生ハードウェア深度
    float zInShadow = tex2D(_LightShadow, uv).x;

    // シャドウバイアスを求める
    float bias = _ShadowNormalBias * tan(acos(dotNL)) + _ShadowBias;

    // ピクセルがシャドウマップの遮蔽者より「奥」なら影。
    // reversed-Z(Metal/DX)は 近い=大きい なので、奥=小さい → zInLVP < zInShadow で影。
    #if UNITY_REVERSED_Z
